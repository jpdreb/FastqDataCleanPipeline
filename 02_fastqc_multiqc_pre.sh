...
run_fastqc() {
    input_dir="$1"
    output_dir="$2"
    echo "A correr FastQC em todos os ficheiros *.fastq.gz em $input_dir..."
...
start_log() {
    log_file="fastqc_multiqc_pre_log_$(date +%Y%m%d_%H%M%S).log"
...
while true; do
    ...
    if [ -d "$rawdata_dir" ]; then
        echo "A pasta $rawdata_dir foi encontrada."
        break
    else
...
fastqc_dir="$base_path/02_output_data/03_fastqc_pre_trimmomatic"
multiqc_dir="$base_path/02_output_data/04_multiqc_pre_trimmomatic"
...
run_fastqc "$rawdata_dir" "$fastqc_dir"
